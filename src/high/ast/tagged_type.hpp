...
std::string primitive_type_to_string(PrimitiveType pt) {
    ...
    case PrimitiveType::ISIZE: return "ISIZE";
    ...
    case PrimitiveType::USIZE: return "USIZE";
...
struct Type {
    ...
    // Constructors
    Type() : tag(Tag::Primitive), _reference_flags(0), primitive(PrimitiveType::Unit) {}

    Type(PrimitiveType t, unsigned char flags)
        : tag(Tag::Primitive), _reference_flags(flags), primitive(t) {}

    Type(const std::string& s, unsigned char flags)
        : tag(Tag::UserDefined), _reference_flags(flags), user_defined(s) {}
    
    Type(const std::vector<Type>& ts, unsigned char flags)
        : tag(Tag::Tuple), _reference_flags(flags), tuple(ts) {}
    
    Type(const Type& t, size_t len, unsigned char flags)
        : tag(Tag::Array), array{new Type(t), len}, _reference_flags(flags) {}

    // Destructor
    ~Type() {
        switch (tag){
            case Tag::UserDefined : user_defined.~basic_string(); break;
            case Tag::Tuple : tuple.~vector(); break;
            default: {}
        }
    }

    // Copy constructor
    Type(const Type& other) : tag(other.tag), _reference_flags(other._reference_flags) {
        switch (tag){
            case Tag::Primitive : primitive = other.primitive; break;
            case Tag::UserDefined : new (&user_defined) std::string(other.user_defined); break;
            case Tag::Tuple : new (&tuple) std::vector(other.tuple); break;
            case Tag::Array : array.t = other.array.t; array.len = other.array.len ; break;
        }
    }

    // Move constructor
    Type(Type&& other) noexcept : tag(other.tag), _reference_flags(other._reference_flags) {
        switch (tag){
            case Tag::Primitive : primitive = other.primitive; break;
            case Tag::UserDefined : new (&user_defined) std::string(std::move(other.user_defined)); break;
            case Tag::Array : array.t = other.array.t; array.len = other.array.len ; break;
            case Tag::Tuple : new (&tuple) std::vector(std::move(other.tuple)); break;
        }
    }
