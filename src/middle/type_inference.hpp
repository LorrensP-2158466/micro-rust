            ...
            class TyInferer {
              ...
              public:
                                           ...
                                           overloaded{
                                               [&](const TypeVar &tv) {
                                                   ...
                                                   if (!t)
                                                       return unknown();
                                                   return std::visit(
                                                       overloaded{[&](const InferTy &) { return resolve(*t); },
                ...
                Ty shallow_resolve(const Ty ty) {
                    ...
                    return std::visit(overloaded{
                                          [&](const TypeVar &tv) {
                                              ...
                                              // instead of {unknown}
                                              return std::visit(
                                                  overloaded{[&](const InferTy &) { return shallow_resolve(*t); },
                ...
                types::Ty from_ast_type(const ast::Type &ast_type) {
                    ...
                    return std::visit(overloaded{
                                          ...
                                          [&](const ast::Type::Tuple &tup) {
                                              ...
                                              std::transform(tup.cbegin(), tup.cend(), std::back_inserter(tys),
                ...
                std::optional<Ty> eq(const Ty &in_t, const Ty &in_u) {
                    ...
                    return std::visit(
                        overloaded{[&](const NeverTy &, const NeverTy &) { return some(t); },
                                   [&](const InferTy &ti, const InferTy &ui) {
                                       return std::visit(
                                           overloaded{[&](const TypeVar &tt, const TypeVar &ut) {
                ...
                FunctionType *create_function_type(const ast::FunDecl &decl) {
                    ...
                    std::transform(decl.args().cbegin(), decl.args().cend(), std::back_inserter(arg_types),
                    ...
                    if (const auto val = std::find_if(seen_args.cbegin(), seen_args.cend(),
                                                      [](const auto &v) { return v.second > 1; });
                        val != seen_args.cend()) {
                        spdlog::critical("Argument `{}` in function `{}`, is declared more than once", val->first,
                                         decl.name());
                ...
                bool is_integer_type(const Ty &ty) {
                    return std::visit(
                        overloaded{[](const NeverTy &) { return true; }, // can be
                                   [&](const InferTy &t) {
                                       return std::visit(overloaded{[](const IntVar &) { return true; },
                                   ...
                                   [](const IntTy &) { return true; }, [](const UIntTy &) { return true; },
