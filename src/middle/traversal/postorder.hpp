...
namespace mr {
    namespace middle {
        namespace traversal {
            ...
            class PostOrder {
              ...
              public:
                explicit PostOrder(const ir::BasicBlocks& blocks)
                    : blocks(blocks), visited(blocks.size()), to_visit() {
                ...
                void visit_block(ir::BlockId bb) {
                    ...
                    to_visit.emplace_back(
                        bb, SuccessorIterator{succs.rbegin(), succs.rend()}
                    );
                }
                ...
                void traverse_successor() {
                    while (!to_visit.empty()) {
                        ...
                        if (succs.current == succs.end) break;
                ...
                class iterator {
                  public:
                    ...
                    iterator& operator++() {
                        ...
                        if (self->to_visit.empty()) self = nullptr;
                        return *this;
                    }
                    ...
                    inline bool operator==(const iterator& other) const {
                        return (self == other.self) &&
                               (self == nullptr || current == other.current);
                    }
                ...
                // Iterator accessors
                iterator begin() {
                    return !blocks.empty() ? iterator(this, to_visit.back().first)
                                           : iterator();
                }
            ...
            static inline auto postorder(const BasicBlocks& blocks) {
                return PostOrder(blocks);
            }

            // static inline auto reverse_postorder(const BasicBlocks& blocks) {
            //     auto iterator = PostOrder(blocks);

            //     std::vector<PostOrder::iterator::value_type> order{};
            //     order.reserve(blocks.size());
            //     order.insert(
            //         order.end(),
            //         std::make_move_iterator(iterator.begin()),
            //         std::make_move_iterator(iterator.end())
            //     );
            //     return std::ranges::reverse(order);
            // }
        } // namespace traversal
