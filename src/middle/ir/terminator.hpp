...
namespace mr { namespace middle { namespace ir {
    ...
    struct GoTo {
        ...
        std::span<const BlockId> successors() const { return std::span(&target, 1); }
    ...
    struct Call {
        ...
        friend std::ostream &operator<<(std::ostream &o, const Call &call) {
            ...
            std::copy(call.args.begin(), call.args.end(), std::ostream_iterator<Operand>(o, ", "));
            o << ") -> " << call.target;
            return o;
        }
    };
    ...
    struct Terminator : terminator_variant_t {
        ...
        std::span<BlockId> successors() {
            return std::visit(overloaded{[&](auto &t) { return t.successors(); }}, *this);
        }
