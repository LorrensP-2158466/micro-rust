...
namespace mr { namespace middle { namespace ir { namespace passes {
    ...
    struct Simplifier {
        ...
        static void remove_dead_blocks(BasicBlocks &blocks) {
            ...
            if (blocks_amt == std::count(reachable.cbegin(), reachable.cend(), true)) {
                return;
            }
        ...
        void simplify() {
            ...
            do {
                ...
                for (const auto bb : blocks.indices()) {
                    ...
                    while (inner_changed) {
                        inner_changed = false;
                        inner_changed |= merge_successors(merged_blocks, terminator);
        ...
        std::optional<ir::Terminator> take_terminator_if_simple_goto(ir::BlockId block) {
            // Example logic: return a Goto terminator if it exists and matches
            // criteria
            const auto &bb = blocks.block(block);
            if (std::holds_alternative<ir::GoTo>(bb.terminator()) && bb.statements.empty()) {
        ...
        void collapse_goto_chain(ir::BlockId &start, bool &changed) {
            ...
            while ((terminator_opt = take_terminator_if_simple_goto(current))) {
                ...
                if (!terminator_opt)
                    throw std::runtime_error("HERE WTF");
    ...
    class Simplify : public IrPass {
      public:
        ...
        void run(Function &body) const override { Simplifier(body._blocks).simplify(); }
    };
