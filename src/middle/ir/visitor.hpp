        ...
        namespace ir {
            ...
            struct PlaceCtx : public std::variant<MutUseCtx, NonMutUseCtx> {
                friend std::ostream &operator<<(std::ostream &o, PlaceCtx cx) {
                    return std::visit(
                        overloaded{[&o](const MutUseCtx &) -> std::ostream & { return (o << "Mut Use"); },
            ...
            class Visitor {
              public:
                ...
                virtual void visit_stmt(const Statement &f, Location l) {
                    std::visit(overloaded{[&](const Assign &a) { visit_assign(a, l); },
                ...
                virtual void visit_print(const SPrintLn p, Location l) {}

                virtual void visit_terminator(const Terminator &term, Location l) {
                    std::visit(
                        overloaded{[&](const GoTo &c) {},
                                   // we have to visit the return local, because we "use" it
                                   [&](const Return &c) { visit_local(RETURN_LOCAL, PlaceCtx(NonMutUseCtx::Move), l); },
                                   [&](const Assert &c) {}, [&](const CondGoTo &cgt) { visit_operand(cgt.op, l); },
                                   [&](const Call &c) {
                ...
                virtual void visit_local(const LocalId &local, PlaceCtx ctx, Location l) {}
                virtual void visit_operand(const Operand &op, Location l) {
                    std::visit(overloaded{[&](const Copy &c) { visit_place(c.val, PlaceCtx(NonMutUseCtx::Copy), l); },
                ...
                virtual void visit_rvalue(const RValue &rv, Location l) {
                    std::visit(overloaded{[&](const AsIs &a) { visit_operand(a.op, l); },
                ...
                virtual void visit_place(const Place &p, PlaceCtx ctx, Location l) { visit_local(p.local, ctx, l); }
            }; // namespace middle
        } // namespace ir
    } // namespace middle
} // namespace mr