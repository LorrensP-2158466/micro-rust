              ...
              private:
                    ...
                    unpack(
                        ...
                        build_in_scope<void>([&](IrBuilder& builder) -> BlockWith<void> {
                            for (const auto& stmt : tast_block._statements) {
                                unpack(
                                    basic_block,
                                    builder.build_statement(*stmt, basic_block)
                                );
                            }
                            ...
                            if (tast_block._tail)
                                unpack(
                                    basic_block,
                                    builder.expr_into_place(
                                        place, *tast_block._tail, basic_block
                                    )
                                );
                ...
                BlockWith<void> build_statement(const tast::Stmt& stmt, BlockId block) {
                    return std::visit(
                        overloaded{
                            [&](const ExprStmt& e) {
                                return build_expr_stmt(e.expr, block);
                            },
                    ...
                    return std::visit(
                        overloaded{
                            [&](const AssignExpr& assign) {
                                const auto rhs =
                                    unpack(block, expr_as_rvalue(*assign.rhs, block));
                                const auto lhs =
                                    unpack(block, expr_as_place(*assign.lhs, block));
                                _blocks.push_assign(
                                    block, std::move(lhs), std::move(rhs)
                                );
                                return block.empty();
                            },
                            [&](const AssignOpExpr& assign) {
                                const auto rhs =
                                    unpack(block, expr_as_operand(*assign.rhs, block));
                                const auto lhs =
                                    unpack(block, expr_as_place(*assign.lhs, block));
                                const auto result = RValue(
                                    BinaryOp{assign.op, Operand::copy(Place(lhs)), rhs}
                                );
                                _blocks.push_assign(
                                    block, std::move(lhs), std::move(result)
                                );
                                return block.empty();
                            },
                            [&](const Break& br) {
                            ...
                            [&](const tast::Return& ret) {
                                ...
                                block = expr_into_place(return_place, *ret.val, block)
                                            .into_block();
                ...
                BlockWith<void> build_print_stmt(const PrintLn& print, BlockId block) {
                    ...
                    auto first_operand =
                        map_optional(print._start_fmt, [&](const std::string& s) {
                    ...
                    for (const auto& [str, name] : print._fmt_structure) {
                        format_structure.emplace_back(
                            str, Operand::copy(local_by_name(name))
                        );
                    }
                    _blocks.push_stmt(
                        block,
                    ...
                    return std::visit(
                        overloaded{
                            [&](const LogicalOpExpr& log_op) {
                                ...
                                const auto cond_op =
                                    unpack(block, expr_as_operand(*log_op.left, block));
                                ...
                                const auto rhs =
                                    expr_into_place(place, *log_op.right, continue_block)
                                        .into_block();
                                ...
                                */
                                ...
                                auto end_then_blk =
                                    build_block_statements(
                                        place, if_el.then_block, begin_then_blk
                                    )
                                        .into_block();
                                ...
                                auto end_else_blk =
                                    if_el.else_block
                                        ? expr_into_place(
                                              place, *(*if_el.else_block), begin_else_blk
                                          )
                                              .into_block()
                                        : begin_else_blk;
                                ...
                                _blocks.terminate(
                                    block,
                                    Terminator(CondGoTo{
                                        cond_op, {begin_else_blk, begin_then_blk}
                                })
                                );
                                ...
                                return build_in_breakable_scope(
                                    ...
                                    [&](IrBuilder& builder) {
                                        const auto body_block =
                                            builder._blocks.create_new_block();
                ...
                BlockWith<Place> expr_as_place(const Expr& expr, BlockId block) {
                    ...
                    return std::visit(
                        overloaded{
                            [&](const FieldExpr& fe) {
                                auto place =
                                    unpack(block, expr_as_place(*fe.expr, block));
                                place.field(fe.idx);
