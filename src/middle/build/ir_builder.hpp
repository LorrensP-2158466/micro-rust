        ...
        namespace build {
            ...
            class IrBuilder {
              ...
              private:
                ...
                BlockWith<void> build_block_statements(const Place place, const BlockExpr &tast_block,
                                                       BlockId basic_block) {
                ...
                BlockWith<void> build_print_stmt(const PrintLn &print, BlockId block) {
                    ...
                    auto first_operand = map_optional(
                        print._start_fmt, [&](const std::string &s) { return Operand::copy(local_by_name(s)); });
                    ...
                    _blocks.push_stmt(block, Statement{FPrintLn{std::move(first_operand), std::move(format_structure),
                                                                std::move(end_str)}});
                ...
                BlockWith<Operand> expr_as_operand(const Expr &expr, BlockId block) {
                    auto handle_temp_move = [&]() {
                        ...
                        const auto temp_place =
                            Place(create_local("temp", std::move(resolved_type), MutabilityType::Immutable));
                ...
                BlockWith<RValue> expr_as_rvalue(const Expr &expr, BlockId block) {
                    return std::visit(
                        overloaded{[&](const BinOpExpr &bin_op) {
                    ...
                    return std::visit(
                        overloaded{
                            ...
                            [&](const CallExpr &call) {
                                ...
                                std::transform(
                                    call.args.cbegin(), call.args.cend(), std::back_inserter(call_operands),
                                    [&](const auto &expr) { return unpack(block, expr_as_operand(*expr, block)); });
                                auto new_block = _blocks.create_new_block();
                                _blocks.terminate(
                                    block, Terminator{Call{call.id, std::move(call_operands), place, new_block}});
                            ...
                            [&](const IfElse &if_el) {
                                ...
                                */
                                ...
                                auto end_else_blk =
                                    if_el.else_block
                                        ? expr_into_place(place, *(*if_el.else_block), begin_else_blk).into_block()
                                        : begin_else_blk;

                                // goto then/else from conditinonal
                                _blocks.terminate(block,
                                                  Terminator(CondGoTo{cond_op, {begin_else_blk, begin_then_blk}}));
                ...
                BlockWith<Place> expr_as_place(const Expr &expr, BlockId block) {
                    ...
                    return std::visit(overloaded{[&](const FieldExpr &fe) {
