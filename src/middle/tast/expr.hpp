...
#define MAKE_EXPR_VARIANT_CONSTR(type)                                                             \
    ...
    struct BlockExpr {
        ...
        BlockExpr(std::vector<U<Stmt>> s, U<Expr> tail)
            : _statements(std::move(s))
            , _tail(std::move(tail)) {}
        BlockExpr(U<Expr> tail)
            : _statements{}
            , _tail(std::move(tail)) {}
    ...
    ENUM_DEFINE(LitIntType, i8, i16, i32, i64, u8, u16, u32, u64, isize, usize, none)
    ...
    struct Float {
        ...
        uint64_t as_raw_value() const {
            if (!suffix || *suffix == LitFloatType::f64)
                return as_double();
            return as_float();
        }
    };
...
#define CREATE_LITERAL_VALUES(val, r_type, c_type)                                                 \
    do {                                                                                           \
        ...
        from_ast_lit(const expr::Literal &lit, inference::TyInferer &inf) {
            ...
            case expr::LiteralKind::Integer: {
                ...
                return {Literal(LiteralKind{Int{v, s}}), t};
            }
            case expr::LiteralKind::Float: {
                ...
                const auto [t, s] =
                    ...
                    default:
                        throw std::runtime_error("invalid suffix with float literal");
                    }
                }();
            }
    ...
    struct IfElse {
        ...
        IfElse(U<Expr> c_expr, BlockExpr tb, OptUnique<Expr> eb)
            : conditional_expr(std::move(c_expr))
    ...
    static inline ir::UnOp un_op_from_ast(expr::UnaryOp op) {
        ...
        case expr::UnaryOp::Negate:
            return ir::UnOp::Neg;
        default:
            throw std::runtime_error("Can't create binary op from borrow/deref expr");
        }
    }
    ...
    struct LogicalOpExpr {
        ...
        friend std::ostream &operator<<(std::ostream &o, const LogicalOpExpr &) { return o; }
    };
    ...
    struct BinOpExpr {
        ...
        friend std::ostream &operator<<(std::ostream &o, const BinOpExpr &as) {
            o << BinOp_to_string(as.op) << "Binary Operation ON " << as.left << " AND " << as.right;
            return o;
        }
    };
