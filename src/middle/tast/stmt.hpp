        ...
        namespace tast {
            ...
            struct PrintLn {
                ...
                static PrintLn from_str(const std::string &format_str) {
                    ...
                    // content)
                    if (format_str[0] == '{' && std::regex_search(searchStart, searchEnd, matches, pattern)) {
                        result._start_fmt = matches[1];  // The first matched placeholder
                        searchStart = matches[0].second; // Move the search start past the first match
                    }
                    ...
                    while (std::regex_search(searchStart, searchEnd, matches, pattern)) {
                        ...
                        result._fmt_structure.push_back({std::string(searchStart, matches[0].first), matches[1]});
                        searchStart = matches[0].second; // Move past the matched placeholder
                    }
                    ...
                    if (searchStart != searchEnd) {
                        ...
                        result._end_str = std::nullopt; // No content after the last placeholder
                    }
            ...
            struct EmptyStmt {};
            using StmtKind = std::variant<EmptyStmt, ExprStmt, LetStmt, PrintLn>;
            struct Stmt {
                StmtKind inner;
                ...
                static inline Stmt empty() noexcept { return Stmt(); }
