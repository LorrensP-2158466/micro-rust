    ...
    struct Scalar {
        ...
        static inline Scalar from_int(SignedIntegral auto value, size_t byte_size) {
            ...
            if (value < 0) {
                ...
                unsigned_value = (~(static_cast<uint64_t>(-value - 1))) & ((1ULL << size) - 1);
            } else {
      ...
      private:
        ...
        template <typename T> T to_int() const {
            ...
            if constexpr (std::is_signed_v<T>) {
                if (signed_value > static_cast<int64_t>(std::numeric_limits<T>::max()) ||
                    signed_value < static_cast<int64_t>(std::numeric_limits<T>::min())) {
                    throw std::runtime_error("Value out of range for target type");
                }
            } else {
                if (signed_value < 0 ||
                    static_cast<uint64_t>(signed_value) > std::numeric_limits<T>::max()) {
                    throw std::runtime_error("Value out of range for target type");
                }
            }
