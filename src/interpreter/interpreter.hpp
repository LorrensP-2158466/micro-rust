...
namespace mr {
    namespace middle_interpreter {
        ...
        class Interpreter {
          ...
          public:
            Interpreter(ir::Ir code) : _code(std::move(code)) {}

            int interp() {

                Value dest{
                    Scalar{0, 0},
                    types::Ty{types::UnitTy{}}
                }; // unit ofcourse, never write to it, but it exists
                init_function_call(
                    ir::Call{
                        "main",
                        std::vector<ir::Operand>(),
                        ir::Place{ir::LocalId{0}},
                        ir::BlockId{0}
                    },
                    dest
                );
            ...
            */
            void init_function_call(
                ...
                std::transform(
                    ...
                    std::back_inserter(values),
                    [&](const ir::Local& l) {
                        return Value(Scalar{0, l.ty.size()}, l.ty);
                    }
                );

                auto frame =
                    Frame(START_LOCATION, fn, std::move(values), dest, caller_target);
            ...
            void return_from_call() {
                ...
                if (caller_goto)
                    _stack.frame().loc =
                        Location{.basic_block = *caller_goto, .stmt_idx = 0};
            }
            ...
            void interp_format(const ir::FPrintLn& format) const {
                ...
                output += map_optional_or(
                    format.start,
                    [&](const ir::Operand& op) {
                        return interp_operand(op).as_program_str();
                    },
                    std::string()
                );
            ...
            void
            interp_rvalue_into_place(const ir::Place& place, const ir::RValue& value) {
            ...
            Value
            binary_bool_op(ir::BinOp op, const Value& left, const Value& right) const {
            ...
            Value
            binary_int_op(ir::BinOp op, const Value& left, const Value& right) const {
                const auto& result_type = left.type;
                if (result_type.is_signed_integral()) {
                    const auto l = left.scalar().to_isize();
                    const auto r = right.scalar().to_isize();
                    const auto cmp_op = [](ir::BinOp op, int64_t l, int64_t r
                                        ) -> std::optional<bool> {
                        switch (op) {
                        case ir::BinOp::Eq:
                    ...
                    if (cmp_op) { return Value::from_bool(*cmp_op); }
                    ...
                    if (overflow) {
                        throw std::runtime_error(
                            fmt::format("Overflow happend on {} and {}", l, r)
                        );
                    }
                    return Value(
                        Scalar::from_int(result, result_type.size()), result_type
                    );
                }
                const auto l = left.scalar().to_usize();
                const auto r = right.scalar().to_usize();
                const auto cmp_op = [](ir::BinOp op, uint64_t l, uint64_t r
                                    ) -> std::optional<bool> {
                    switch (op) {
                    case ir::BinOp::Eq:
                ...
                return std::visit(
                    overloaded{
                        ...
                        [&](const types::IntTy&) -> Value {
                            ...
                            case ir::UnOp::Not:
                                return Value(
                                    Scalar::from_int(~i_val, val.type.size()), val.type
                                );
                            case ir::UnOp::Neg:
                                return Value(
                                    Scalar::from_int(-i_val, val.type.size()), val.type
                        ...
                        [&](const types::UIntTy&) -> Value {
                            ...
                            case ir::UnOp::Not:
                                return Value(
                                    Scalar::from_uint(!u_val, val.type.size()), val.type
                                );
                            default:
                                throw std::runtime_error("Invalid UnOp on integer");
                            }
            ...
            void go_to_block(ir::BlockId target) {
                _stack.frame().loc = Location{.basic_block = target, .stmt_idx = 0};
            }
