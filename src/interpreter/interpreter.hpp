    ...
    namespace middle_interpreter {
        ...
        class Interpreter {
          ...
          private:
            ...
            */
            void init_function_call(const ir::Call &call, Value &dest,
                ...
                std::transform(call.args.cbegin(), call.args.cend(), std::back_inserter(values),
                               [&](const ir::Operand &op) { return interp_operand(op); });

                std::transform(fn.fn_locals().begin(), fn.fn_locals().end(), std::back_inserter(values),
            ...
            void interp_stmt(const ir::Statement &stmt) {
                std::visit(
                    overloaded{[&](const ir::Assign &assign) { interp_rvalue_into_place(assign.place, assign.value); },
            ...
            void interp_format(const ir::FPrintLn &format) {
                ...
                output += map_optional_or(
                    format.start, [&](const ir::Operand &op) { return interp_operand(op).as_program_str(); },
                    std::string());
            ...
            void interp_terminator(const ir::Terminator term) {
                std::visit(overloaded{[&](const ir::GoTo &gt) { go_to_block(gt.target); },
            ...
            void interp_rvalue_into_place(const ir::Place &place, const ir::RValue &value) {
                ...
                place_value = std::visit(overloaded{[&](const ir::AsIs &as_is) { return interp_operand(as_is.op); },
            ...
            Value interp_bin_op(const ir::BinaryOp &bin_op) {
                ...
                return std::visit(
                    overloaded{[&](const types::IntTy &) { return binary_int_op(bin_op.op, left, right); },
            ...
            Value interp_un_op(ir::UnOp op, const Value &val) const {
                return std::visit(
                    overloaded{[&](const types::BoolTy &) -> Value {
            ...
            Value interp_operand(const ir::Operand &op) {
                return std::visit(overloaded{[&](const ir::Const &v) { return Value(v.scalar, v.ty); },
            ...
            Value uninit_val_of_type(const types::Ty &type) {
                return std::visit(overloaded{[&](const types::TupleTy &tup) {
                                                 std::vector<Value> vals;
