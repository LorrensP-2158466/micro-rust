    ...
    namespace middle
    {
        ...
        class SymbolTable{
            struct ScopeEntry{
                ...
                // raw pointer is valid because this scope can only exist because of it's parent scope
                // Parent scope is NULl so then we know we are at the end when searching
                ScopeEntry(){
                ...
                T& get_value(const std::string& symbol){
        ...
        public:
            ...
            const auto& get_current_scope() const{
                return _scopes.back();
            }

            auto& get_current_scope() {
                return _scopes.back();
            }

            void push_scope() {
                _scopes.push_back(ScopeEntry());
            }
